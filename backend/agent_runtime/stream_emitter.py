...
class StreamEmitter:
    ...
    def emit_delta(self, channel: str, text: str) -> None:
        ...
        if len(text) <= CHUNK_SIZE:
            ...
            for i, chunk in enumerate(chunks):
                _write({"type": "model_delta", "channel": channel, "text": chunk})
                if i < len(chunks) - 1:
    ...
    def emit_intent(self, call_id: str, name: str, args: str) -> None:
        ...
        _write({"type": "tool.intent", "callId": call_id, "name": name, "args": args})
    ...
    def emit_final(self, content: str = "", reasoning: str = "") -> None:
        # Content was already streamed via emit_delta from TextBlock handler.
        # Only send the closing signal.
        _write({"type": "model_final", "content": "", "reasoningContent": reasoning})
