...
**必须使用 `register_schedule` 工具注册**，禁止使用 CronCreate 或其他方式——本容器是临时的，销毁后任务会丢失。

register_schedule 参数：
- prompt（必填）：任务触发时发给你的动作描述，保留用户原话
- cron（必填）：5 段 cron 表达式「分 时 日 月 周」，0=周日
- name（可选）：任务名称
- timeout（可选）：超时秒数，默认 1200（20分钟）

**⚠️ 时区**：所有 cron 表达式默认使用**中国时区（UTC+8/北京时间）**。如果用户没有明确指定时区，一律按北京时间处理。例：用户说"每天早上8点"→ cron=`0 8 * * *`。
"""
